...
export default function AccountPage() { // Đổi tên hàm cho rõ ràng
  ...
  const orderCounts = {
    ...
    review: orders.filter(o => o.orderStatus === 'delivered').length,
  };

  const statusItems = [
    { label: 'Chờ xác nhận', icon: ClockSolid, count: orderCounts.pending, filter: 'pending' },
    { label: 'Chờ lấy hàng', icon: ShoppingBagSolid, count: orderCounts.confirmed, filter: 'confirmed' },
    { label: 'Chờ giao hàng', icon: TruckSolid, count: orderCounts.shipping, filter: 'shipping' },
    { label: 'Đã giao', icon: CheckCircleSolid, count: orderCounts.delivered, filter: 'delivered' },
    { label: 'Đánh giá', icon: StarSolid, count: orderCounts.review, filter: 'review' },
  ];
          ...
          {loading ? (
            ...
            <div className="flex justify-between gap-2">
              {statusItems.map((item) => (
                <Link
                  key={item.label}
                  href={`/account/orders?filter=${item.filter}`}
